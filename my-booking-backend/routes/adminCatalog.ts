...
router.post('/g-services', async (req, res) => {
  try {
    ...
    const newService = new Service({ title, description, price, duration, category }) // category hinzugefügt
...
router.put('/g-services/:id', async (req, res) => {
  try {
    ...
    const updatedService = await Service.findByIdAndUpdate(
      req.params.id,
      { title, description, price, duration, category }, // category hinzugefügt
      { new: true }
    )
